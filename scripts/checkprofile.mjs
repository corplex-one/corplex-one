...
await page.evaluate(([d,n,r])=>{window.__DATA=d;window.__ME=n;window.__ROLES=r;window.__calls=[];
  ...
  const bt=document.getElementById('boot');if(bt)bt.remove();},[DATA,who,DATA._roles[who]]);
...
console.log(await page.evaluate(()=>{
  ...
  return {rows, panels,
    ...
    // the birthday stopped being the employee's to type in Sep 2026
    bdayReadOnly: !document.getElementById('pf_bdayD') && !document.querySelector('#pf_bday')
      && !!document.querySelector('.pfread'),
