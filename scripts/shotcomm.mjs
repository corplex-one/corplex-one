...
for(const who of ['Shohruh Karimov','Avin Mascarenhas']){
  const {p, errs} = await open(who, 1700, 1700);
  await p.evaluate(() => { state.mode='staff'; state.tab='commission'; render(); });
  await p.waitForTimeout(400);
  await shot(p, 'c-' + who.split(' ')[0].toLowerCase() + '-commission.png');
  if(errs.length) console.log('  !! ' + errs[0]);
  await p.close();
}
{
  const {p, errs} = await open('Avin Mascarenhas', 1700, 1200);
  await p.evaluate(() => { state.mode='staff'; state.tab='payment'; render(); });
  await p.waitForTimeout(400);
  await shot(p, 'c-payment.png');
  if(errs.length) console.log('  !! ' + errs[0]);
  await p.close();
}
